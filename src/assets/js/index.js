...
const footerLang = document.querySelectorAll(".footer-lang__content-item");
const footerCurrent = document.querySelector(".footer-lang__current");
...
const handleFaqItem = ({ currentTarget: target }) => {
  ...
  content.style.height = `${isOpened ? paragraph.clientHeight : 0}px`;
};
...
const setTexts = () => {
  ...
  // Обновление текста в footer
  footerCurrent.innerText = lang === 'en' ? 'english' : 'русский';

  // Обновление всех элементов с data-lang
...
setTexts();
initSlider();
startTimer("November 27, 2025 00:00:00");
...
footerLang.forEach(lang => lang.addEventListener("click", toggleLanguage));
